...
eye_left = [deepcopy(img[y:y+SIZE, x:x+SIZE]) for y in (BIAS_Y+SIZE*2,) for x in range(BIAS_X, img.shape[1], SIZE) if x+SIZE <= img.shape[1]]
eye_right = [deepcopy(im[:, ::-1]) for im in eye_left]

for i, eye in enumerate( zip(eye_left, eye_right) ):
    img_eye = np.concatenate(eye, axis=1)
    Image.fromarray(img_eye.astype(np.uint8)).save(f"110_eye_{i}.png")
...
Image.fromarray(img.astype(np.uint8)).save("110_mask.png")